...
import { Radio, Button, Select, Option, TextInput, Checkbox } from 'jimu-ui'; // NEW: Import Checkbox
...
// Keep QueryTask and Query imports as they are needed for future steps, but not actively used here yet.
...
const Widget = (props: AllWidgetProps<unknown>) => {
  ...
  // NEW: State to store the IDs of selected layers
  const [selectedLayerIds, setSelectedLayerIds] = useState<string[]>([]);
  // NEW: State to control the visibility of the layer selection checkboxes
  ...
  useEffect(() => {
    if (jimuMapView && jimuMapView.view) {
      ...
      console.log('Available Feature Layers:', layers); // Log the detected layers
      // NEW: Pre-select all available layers by default
  ...
  // handleSave remains as in Step 1, logging the buffer graphic
  const handleSave = () => {
    if (bufferGraphic) {
      console.log('Current Buffer Graphic:', bufferGraphic);
      // NEW: Also log the currently selected layers for verification
      console.log('Currently Selected Layers:', selectedLayerIds);
    } else {
      console.log('No buffer graphic to save.');
    }
  };

  // NEW: Handler for toggling layer selection visibility
    ...
    >
      ...
      {jimuMapView && (
        <>
          ...
          {/* NEW: Button to toggle layer selection visibility */}
          ...
          {/* NEW: Layer selection dropdown/section */}
          {showLayerSelection && (
            <div style={{ 
              ...
              padding: '0.5rem', 
              marginBottom: '1rem',
              maxHeight: '150px', // Limit height for scrollability
