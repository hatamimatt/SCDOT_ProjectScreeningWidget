...
const Widget = (props: AllWidgetProps<unknown>) => {
  ...
  // NEW: State to store the most recently created buffer graphic
    ...
    if (jimuMapView && jimuMapView.view) {
      ...
      sketchVM.on('create', (event) => {
        if (event.state === 'complete') {
          ...
          if (buffer) {
            const newBufferGraphic = new Graphic({ // Changed variable name to newBufferGraphic
              geometry: buffer,
            ...
            layer.add(newBufferGraphic); // Add the new buffer graphic
            setBufferGraphic(newBufferGraphic); // NEW: Store the buffer graphic in state
  ...
  const handleDrawClick = () => {
    if (sketchViewModel) {
      // NEW: Clear previous buffer graphic when starting a new draw
  ...
  const handleClear = () => {
    if (graphicsLayer) {
      ...
      setBufferGraphic(null); // NEW: Clear the stored buffer graphic
    }
  };

  const handleSave = () => {
    // MODIFIED: Now logs the buffer graphic to console instead of alert
    if (bufferGraphic) {
